#!/usr/bin/env python3
"""Extract Palo Alto PCNSE exam questions and generate HTML page."""
...
DB_CONFIG = {
    'host': 'localhost',
    'port': 5432,
    'database': 'CERTIFICATIONBABA',
    'user': 'postgres',
    'password': 'admin@123'
}

def get_pcnse_questions():
    """Fetch all PCNSE questions with associated data."""
    ...
    print(f"Found {len(questions)} questions for PCNSE")
...
def copy_images(questions_data, base_folder):
    """Copy images to organized folder structure."""
    print(f"\nCopying images to {base_folder}...")
    ...
    for q in questions_data:
        ...
        question_folder = os.path.join(base_folder, 'PCNSE', str(discussion_id))
        ...
        for img in q['images']:
            ...
            if local_path and os.path.exists(local_path):
                dest_path = os.path.join(question_folder, file_name)
                try:
                    shutil.copy2(local_path, dest_path)
                    total_images += 1
                    print(f"  Copied: {file_name} -> {question_folder}")
    ...
    return total_images, questions_with_images
...
<head>
    ...
    <title>{escape_html(exam_code.upper())} - {escape_html(exam_name)}</title>
    <style>
        body {{
        ...
        h1 {{
            color: #2c3e50;
            border-bottom: 3px solid #e84034;
...
<body>
    <div class="container">
        <h1>{escape_html(exam_name)} - All Questions and Answers <span class="provider-badge">Palo Alto Networks</span></h1>
        <div class="exam-info">
            <strong>Exam Code:</strong> {escape_html(exam_code.upper())}<br>
            <strong>Exam Name:</strong> {escape_html(exam_name)}<br>
            <strong>Provider:</strong> Palo Alto Networks<br>
...
"""
        ...
        if q['images']:
            ...
            for img in q['images']:
                img_type, original_url, local_path, file_name = img
                discussion_id = q['discussion_id']
                if discussion_id and file_name:
                    # Use relative path from HTML file
                    rel_path = f"PCNSE/{discussion_id}/{file_name}"
                    html += f'                        <img src="{rel_path}" alt="{img_type}" title="{img_type}">\n'
            html += '                    </div>\n'
...
def main():
    print("="*80)
    print("Palo Alto Networks PCNSE Exam - Data Extraction Tool")
    print("="*80)

    print("\nExtracting PCNSE questions from database...")
    questions_data, exam_code, exam_name = get_pcnse_questions()
    ...
    base_folder = 'Palo_Alto_PCNSE_Exam'
    total_images, questions_with_images = copy_images(questions_data, base_folder)
    ...
    print(f"\nSummary:")
    print(f"  - Exam: {exam_code.upper()} - {exam_name}")
    print(f"  - Provider: Palo Alto Networks")
    print(f"  - Total Questions: {len(questions_data)}")
    print(f"  - Total Images: {total_images}")
    print(f"  - Questions with Images: {questions_with_images}")
    print(f"\nTo view the exam questions, open: {output_file}")
    print(f"{'='*80}\n")
